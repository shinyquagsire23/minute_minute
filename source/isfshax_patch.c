...
bool isfshax_patch_apply(u32 fw_img_start){
    ...
    BOOT1_serial(0x8D46400 + success);
    ...
    static const u32 adds4 = 0x32044bcc;
    success = isfshax_search_patch(fw_img_start, end, ios_reload_branch, sizeof(ios_reload_branch), 0, &adds4, sizeof(adds4));
    BOOT1_serial(0x8D46500 + success);
    printf("Reload patch: %i\n", success);

    // Shutdown properly instead of going to Standby Mode (DRAM on)
    static const u8 shutdownStuff[] = { 0x23, 0x80, 0x68, 0x10, 0x02, 0x1b, 0x42, 0x98, 0xd0, 0x0a, 0x23, 0xa3, 0x00, 0x9b, 0x58, 0xe3, 0x2b, 0x00, 0xd0, 0x04, 0x68, 0xfa, 0x03, 0xd2, 0xd4, 0x01, 0x20, 0x04, 0xe0, 0x00 };
    success = isfshax_search_patch(fw_img_start, end, shutdownStuff, sizeof(shutdownStuff), 26, &movr01, sizeof(movr01));
    BOOT1_serial(0x8D46400 + success);
    printf("Reboot patch: %i\n", success);
